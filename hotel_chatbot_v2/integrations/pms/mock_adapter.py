...
class MockPMSAdapter:
    ...
    def __init__(self) -> None:
        today = date.today()
        self.guests = {
            "g1": Guest("g1", "John Martin"),
            "g2": Guest("g2", "Anna Silva"),
        }
        ...
        self.incidents: dict[str, Incident] = {
            "i1": Incident(
                "i1", "214", "MAINTENANCE", "Air conditioning not working", IncidentStatus.OPEN, datetime.now()
            )
        }
    ...
    def get_incidents(self, status: str | None = None) -> list[Incident]:
        ...
        if status:
            ...
            incidents = [incident for incident in incidents if incident.status.value == normalized]
        return incidents
